...
void setup() {
  ...
  pinMode(lights_on, OUTPUT);
  pinMode(lights_off, OUTPUT);
  head.attach(11);
  turn(angle);
  ...
  while (bluetooth.available()) {                    //this while condition will execute indefinitely
    ...
    if (isdigit(data)) {                    //if readData is digit
      ...
      if (handbrake_stat == 0 && directionSwitch == 1) {
        analogWrite(motors_forward, sped);
      } else if (handbrake_stat == 0 && directionSwitch == 2) {
        analogWrite(motors_back, sped);
      }
      readstring = "";
    }
...
void headlightOn() {
  ...
  digitalWrite(lights_on, HIGH);
}

void headlightOff() {
  //make headlights pin LOW
  digitalWrite(lights_on, LOW);
}
